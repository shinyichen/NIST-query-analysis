...
# inputs
result_dir = '../NIST-SPARQL-Evaluation-v2.3r/M18-data/output/ta1-cmu'
out_dir = 'output/cmu/ta1'
                    ...
                    if 'AIDA_TA1_CL' in file:
                        with open(ttl_out_dir + '/' + file + '.html', 'w') as out_file_class:
                            ...
                            for row in reader:
                                ...
                                ''' % (row['?docid'],
                                       row['?query_type'].replace(ldcOnt, 'ldcOnt:')[1:-1],
                                       row['?cluster'][1:-1],
                                       row['?type'].replace(ldcOnt, 'ldcOnt:')[1:-1],
                                       mention_tok,
                                       mention_txt,
                                       row['?t_cv'],
                                       row['?cm_cv'],
